...
public class MyUnitTest {
	...
	public void test() throws InterruptedException {
		List<Deployment> deployment = flowableRule.getRepositoryService().createDeploymentQuery().list();
		for (Deployment deploymentVar : deployment) {
			System.err.println("Deployment " + deploymentVar.getId());
		}
		List<ProcessDefinition> procDef = flowableRule.getRepositoryService().createProcessDefinitionQuery()
				.deploymentId(deployment.get(0).getId()).list();
		for (ProcessDefinition processDefinition : procDef) {
			System.out.println("Process def " + processDefinition.getKey());
		}

		System.out.println("Starting the process with deployment id: " + procDef.get(0).getName());
		runProcess();
	}
